...
export type ApplySuggestion =
  ...
  /** Parameter Database — remove multiple interactions at once */
  | { type: "remove_interactions_bulk"; interactions: Array<{ sourceParameterName: string; targetParameterName: string }> }
...
- Coach asks you to link new methods to a parameter (or a batch of parameters) with rationale/evidence: every new link you create MUST carry evidenceQuality alongside its evidence in add_parameter_method/add_parameter_methods_bulk, in the same action — this is easy to forget on long lists, so double-check every entry before emitting, even in a 15+ entry bulk action.
...
- configure_mesocycles: {"type":"configure_mesocycles","mesocycles":[{"name":"Mesocycle 1","microcycles":[{"duration":7,"intensity":"2"},{"duration":7,"intensity":"5"},{"duration":7,"intensity":"7"},{"duration":7,"intensity":"1"}]},{"name":"Mesocycle 2","microcycles":[{"duration":7,"intensity":"5"},{"duration":7,"intensity":"7"},{"duration":5,"intensity":"9"},{"duration":2,"intensity":"1"}]}]}
  Full structure replacement — use when microcycles have different durations or when adding/removing mesocycles or microcycles. Duration is in days. Intensity is optional (defaults to "5"). Valid intensities (Borg CR10): "0" (Rest), "1" (Very Easy), "2" (Easy), "3" (Moderate), "4" (Somewhat Hard), "5" (Hard), "6" (Hard+), "7" (Very Hard), "8" (Very Hard+), "9" (Extremely Hard), "10" (Maximal).
...
- set_microcycle_intensities: {"type":"set_microcycle_intensities","plan":[{"mesocycleName":"Mesocycle 1","mesoIntensity":"7","intensities":["2","5","7","1"]},{"mesocycleName":"Mesocycle 2","mesoIntensity":"9","intensities":["5","7","9","1"]}]}
  Valid intensity values (Borg CR10): "0" (Rest), "1" (Very Easy), "2" (Easy), "3" (Moderate), "4" (Somewhat Hard), "5" (Hard), "6" (Hard+), "7" (Very Hard), "8" (Very Hard+), "9" (Extremely Hard), "10" (Maximal).
...
- set_daily_intensities: {"type":"set_daily_intensities","plan":[{"mesocycleName":"Mesocycle 1","microcycleIndex":1,"days":["2","5","7","0","5","7","0"]},{"mesocycleName":"Mesocycle 1","microcycleIndex":2,"days":["5","7","9","0","7","9","0"]}]}
  Sets the intensity for each individual training day within a microcycle. microcycleIndex is 1-based (1 = first microcycle of that mesocycle). days array length must match the microcycle duration in days — provide one value per day in calendar order. Valid values (Borg CR10): "0" (Rest), "1" (Very Easy), "2" (Easy), "3" (Moderate), "4" (Somewhat Hard), "5" (Hard), "6" (Hard+), "7" (Very Hard), "8" (Very Hard+), "9" (Extremely Hard), "10" (Maximal).
- set_periodization: {"type":"set_periodization","entries":[{"methodName":"<exact name>","mesocycleName":"Mesocycle 1","microcycleIndex":1,"frequency":3,"sets":4,"reps":"5","intensity":"82","extraParams":{"Organization":"Whole","Contrast":"No"}}]}
  ...
  ⚠️ reps and intensity render in a plain number input in the periodization table (with a separate unit dropdown next to it, e.g. "%" or "1RM") — the value MUST be a single plain number as a string, nothing else. Never write a range ("3-5"), a unit, or descriptive text into the value ("80-85% 1RM", "82%", "RPE 8" are all invalid and will silently fail to display — the number input cannot render them). If you want to express a range or a qualifier, pick the single representative number for the field (e.g. the top of the range, or the number you'd actually coach to) and say the full range/qualifier in your chat reply instead. This applies equally to any quantitative extraParams value.
  ALL parameters listed for that method in the "Configurable parameters per method" section of the context (both qualitative/dropdown AND quantitative, e.g. rest durations, rep distance) MUST be included in extraParams. Copy the parameter name character-for-character from that list as the extraParams key — do not paraphrase, abbreviate, translate, or append the unit to the key (units are informational only, not part of the key). For qualitative/dropdown parameters, pick one of the listed options as the value. For quantitative parameters, provide a number (e.g. 90 for seconds). Do not skip any parameter listed for that method.
  ⚠️ Coaches name their method parameters however they like — "Frequency"/"Sets"/"Reps"/"Intensity" are NOT guaranteed names, they are just common ones. The typed frequency/sets/reps/intensity fields above are a convenience ONLY for methods whose parameter list contains an entry named EXACTLY "Frequency"/"Sets"/"Reps"/"Intensity" — check that method's own list in "Configurable parameters per method" before using them. This matters most for intensity: many methods express load/effort through a differently-named parameter instead (e.g. "Velocity Threshold", "Resistance", "% 1RM", "RPE", "Work Intensity") — if the method's list has no parameter literally named "Intensity", do NOT use the intensity field at all; instead set the appropriate load/effort parameter from that method's own list via extraParams under its real name. The same applies to sets/reps/frequency if a method's list lacks that exact name.
...
- save_as_template: {"type":"save_as_template","methodName":"<exact method name>","templateName":"<name for the new template>","mesocycleName":"Mesocycle 1"}
  Saves the current periodization table values for a method as a new named programming template. Only available at Step 4.
  "mesocycleName" is optional — include it to save only that mesocycle's values (produces fewer columns); omit it to save all mesocycles' values.
  SCOPE RULE: If the coach specifies a source mesocycle (e.g. "save Mesocycle 1 as a template"), include mesocycleName. If they do not specify, ask whether they want to save one mesocycle or the full plan before generating the action — do not assume all.
...
- set_day_intensity: {"type":"set_day_intensity","dayDate":"YYYY-MM-DD","intensity":"<intensity level>"}
  Changes the intensity of a training day. Valid values (Borg CR10): "0" (Rest), "1" (Very Easy), "2" (Easy), "3" (Moderate), "4" (Somewhat Hard), "5" (Hard), "6" (Hard+), "7" (Very Hard), "8" (Very Hard+), "9" (Extremely Hard), "10" (Maximal). Use the exact YYYY-MM-DD date from context.
...
- add_parameter_method: {"type":"add_parameter_method","parameterName":"<exact name of existing parameter>","methodId":"<exact method ID from wizard context>","rationale":"<why this method improves this parameter>","evidence":"<research citations or supporting evidence>","evidenceQuality":"<REQUIRED whenever evidence is present: strong|moderate|preliminary|expert_opinion>"}
  Only use method IDs as listed in the wizard context. Do not invent method IDs.
  evidenceQuality rates the strength of the evidence using GRADE-inspired levels (see update_parameter_method below for definitions). NON-NEGOTIABLE: if this entry has an "evidence" value, it MUST also have an "evidenceQuality" value in the exact same entry — an entry with evidence but no evidenceQuality is an incomplete, incorrect action. Never leave it for a follow-up update.
...
- add_parameter_methods_bulk: {"type":"add_parameter_methods_bulk","links":[{"parameterName":"<exact parameter name>","methodId":"<exact method ID from wizard context>","rationale":"<...>","evidence":"<research citations>","evidenceQuality":"<REQUIRED whenever evidence is present: strong|moderate|preliminary|expert_opinion>"},{"parameterName":"<exact parameter name>","methodId":"<exact method ID from wizard context>"}]}
  ...
  evidenceQuality rates the strength of the evidence using GRADE-inspired levels (see update_parameter_method below for definitions). NON-NEGOTIABLE, PER ENTRY: every single entry in "links" that has an "evidence" value MUST also carry its own "evidenceQuality" value — this applies to EVERY entry in the array, not just the first few. In long lists (10+ entries) it is a common mistake to write evidence for every entry but only rate quality on some — before emitting the action, re-check each entry individually and confirm it has both fields together, or neither. Never leave evidenceQuality for a follow-up update when you already have the evidence in hand.
...
- remove_interactions_bulk: {"type":"remove_interactions_bulk","interactions":[{"sourceParameterName":"<exact name>","targetParameterName":"<exact name>"},{"sourceParameterName":"<exact name>","targetParameterName":"<exact name>"}]}
  Removes multiple interactions at once. Preferred over remove_interaction when removing 2 or more. Always confirm with the coach before emitting. Use exact names from the "Interactions already defined" index.
...
function getSuggestionPreview(action: ApplySuggestion): string {
  switch (action.type) {
    ...
    case "remove_interactions_bulk":
      return `Remove ${action.interactions.length} interaction${action.interactions.length !== 1 ? "s" : ""}: ${action.interactions.map((i) => `${i.sourceParameterName} → ${i.targetParameterName}`).join(", ")}`;

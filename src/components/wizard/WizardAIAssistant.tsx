...
- assign_exercises: {"type":"assign_exercises","replace":false,"assignments":[{"methodName":"<exact method name>","mesocycleName":"Mesocycle 1","categoryName":"<exercise category — include ONLY if method has categories, omit otherwise>","exercises":[{"exerciseId":"<exact id>","exerciseName":"<name>","libraryId":"<exact library id>"}]}]}
  IMPORTANT: This assigns exercises from the existing database to a method in the plan. You CAN do this — it does NOT modify the exercise database, it only selects which exercises to use for a method. Do NOT tell the coach to use a button or add exercises manually when exercises are listed in the wizard context. Use exact exerciseId and libraryId values from the "Available exercises" section in the wizard context — never invent IDs. Set "replace": true to replace the current selection; omit or false to append. If the method context lists "Categories:", produce one assignment entry per category. If the coach asks for exercises that are NOT listed in the wizard context (not in the database), tell them to add those exercises to the Exercise Database first — then you can assign them.

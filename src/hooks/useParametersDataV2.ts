...
export function useParametersDataV2() {
  ...
  const addParameterMethod = useCallback(async (parameterId: string, methodId: string, rationale?: string, evidence?: string, evidenceQuality?: string) => {
    const exists = data.parameterMethods.some(m => m.parameterId === parameterId && m.methodId === methodId);
    if (exists) return;
    const newMethod: ParameterMethodV2 = { id: Date.now().toString(), parameterId, methodId, rationale, evidence, evidenceQuality };
  ...
  const addParameterMethodsBulk = useCallback(async (
    entries: Array<{ parameterId: string; methodId: string; rationale?: string; evidence?: string; evidenceQuality?: string }>,
  ) => {
    const existing = new Set(

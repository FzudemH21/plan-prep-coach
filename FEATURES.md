...
|---|---|---|
| 🔴 Now | **Coach app translation (German / English)** — i18n infrastructure done, language picker done. **Coach mobile fully translated:** CoachProfilePage, AthleteAppLayout, Login/Signup/Home/SessionLibrary, CoachMobileLayout + athletes/profile/messages/training/notifications pages, CoachAthleteProgressTab + CoachAthleteSettingsTab, CoachMobileAssignProgramPage, CoachMobileSessionEditPage, CoachMobileSessionLoggingPage (partial — swap dialog may have a few remaining strings). **Still to do:** coach desktop pages (`AthleteDatabase`, `AthleticismDatabaseV2`, `CoachMessagesPage`, `ExerciseLibrariesPage`, `MacrocyclePage`, `MesocyclePage`, `MicrocyclePlanningPage`, `ToolboxDatabase`, `TrainingProgramsPage`). | 🚧 In Progress |
| 🔴 Now | **Coach onboarding flow** — guided "set up your first athlete and plan" walkthrough shown after initial login. Steps: create/invite an athlete → open the wizard and follow step 1 → optional: upload a document to the knowledge base. Reduces time-to-value for new coaches and prevents the blank-state drop-off. | ⬜ Open |
| 🟡 Near | **GDPR / health data compliance** — wellness check-ins, pain maps, illness questionnaires, and anamnesis records (medical history, movement screening) all constitute health data under GDPR Art. 9. **AI provider DPAs (do today, free):** Sign Anthropic DPA at console.anthropic.com → Settings → Privacy; sign OpenAI DPA at platform.openai.com → Settings. Both providers retain API data for up to 30 days for safety review by default and do not train on API data. **HIPAA note:** Anthropic currently offers no BAA — cannot legally process US PHI through the Claude API without one. OpenAI does offer a BAA (must be explicitly requested). Until a BAA is in place, avoid sending identifiable health data to the AI; use athlete IDs and anonymise where possible. **Supabase:** Sign Supabase DPA (dashboard, no cost); configure project to EU region (Frankfurt) before any EU-facing launch. **App changes required before launch:** (1) Add explicit athlete consent step in `AthleteConnectPage` / monitoring enrollment — coach-side toggle is not sufficient under GDPR. (2) Write privacy policy covering health data categories, AI processing, 30-day API retention, and athlete deletion rights. (3) Ensure full data deletion cascade when an athlete connection is removed. (4) Move OpenAI embedding calls server-side (Edge Function) so health data in RAG queries never leaves the server unguarded — currently the OpenAI key is still client-side (see security hardening entry). (5) Consider anonymising health data before sending to AI (strip names/DOB, use connection IDs). | ⬜ Open |
| 🔵 Future | **Pre-publish security hardening** — ✅ Anthropic API key moved to Supabase Edge Function (`supabase/functions/ai-proxy/`); `dangerouslyAllowBrowser` removed; client calls proxy via JWT-authenticated fetch. **Still open:** per-coach AI usage metering, rate limiting, full RLS audit, move OpenAI embedding key to edge function. | 🚧 In Progress |
...
| 🔵 Future | **Group / squad programming** — assign one plan to multiple athletes simultaneously. At assignment, each athlete gets their own copy of the plan in `athlete_schedule`. Where an exercise has a known e1RM for a given athlete (from `athlete_test_results` / `athlete_session_logs`), auto-populate planned load using the Epley formula and the percentage from the periodization table. Each athlete's copy adjusts to their individual strength. Prerequisite for team-sport market (rugby, football, basketball). | ⬜ Open |
...
| 🔵 Future | **Multi-coach / club licence tier** — one club account, multiple coach logins, shared athlete database with role-based access (head coach / assistant). Enterprise pricing tier. | ⬜ Open |
| 🔵 Future | **Periodization template marketplace** — coaches can publish and optionally sell their method configurations and periodization templates. Network effect + content moat without hiring content creators. | ⬜ Open |
...
| Daily check-in (athlete app) | McLean 5-item wellness (auto-advance + confirm screen), body map pain selection with inline NRS rating (dot markers, severity colours), OSTRC-H illness symptom checklist (26 items) + overall NRS. Single centered Dialog — no re-animation between steps. Stored in `athlete_daily_checkins` (Supabase). Coach can enable/disable per athlete via Daily Monitoring toggle in Settings tab. Auto-popup logic lives in `AthleteAppLayout` (app shell, mounted for every tab) rather than the Today page, so it fires once per day on first app open regardless of which tab the athlete lands on. Manual re-trigger: "Log check-in" / "Edit check-in" pill next to the Today tab greeting reopens the sheet on demand (shared via `useOutletContext`); re-submitting overwrites today's row (upsert on `athlete_connection_id, date`). Reopening now prefills every field from today's already-saved entry (wellness, pain map, illness, notes, custom metrics) instead of starting blank — `todayCheckin` + `todayCustomMetricValues` (latest `athlete_test_results` row per parameter for today, since those are inserted rather than upserted) are passed into `DailyCheckinSheet` as `existingCheckin`/`existingCustomMetricValues` and used to seed state on open. Only today's entry is editable; past days are read-only to preserve trend history. |
| Athlete Profile — Monitoring tab | Second tab (opens by default) in athlete profile. 2×2 grid layout: Wellness score card (composite + z-score, expandable 5-item breakdown, ← → date navigator + calendar popover to step through past check-ins) · Illness card · Wellness Trend chart (z-score line + 7-day trailing MA, ±1 SD shaded band, date range picker + 7/14/28/90d quick buttons) · Pain card (full body map front+back with NRS dots, severity list). Transparent body map PNGs. All 4 cards reflect selected day. Coach-side data fetched via `athlete_connection_id`. Date navigator now shows every calendar day back to the athlete's first check-in (not just days with a logged entry) — skipped days appear as a dash ("–") placeholder across all three cards instead of silently being skipped over. |
...
| **Coach Mobile App — athlete overview + session edit** | Mobile-first coach app at `/coach-mobile`. Athlete list with connection status. Per-athlete profile: training calendar (week navigator, session cards with intensity badge, completed-session green display matching athlete app: bg-green-50/border, "Completed · X min · RPE X · sRPE: X AU"). Session edit page: view/edit toggle, inline set table, parameter config sheet (filtered — no `_unit` params), add exercise from library, ⋮ menu per exercise (Details / Change Exercise / Duplicate / Delete), exercise name click → detail sheet (video + description), ExercisePickerSheet centered on desktop. Supabase-backed via `athlete_session_logs`. Overview tab: Info card → Monitoring (Wellness/Pain/Illness/custom-metric cards, today-only — no day-by-day navigator) → Notes card moved to the very bottom of the tab. `WellnessMiniChart` trend now plots every calendar day in the selected window (7/14/28/90d), not just days with a logged check-in — missing days render as a real gap in the line (`connectNulls={false}`) instead of silently connecting the nearest two logged days. Notes card switched from the legacy single `notes` string to the same `notesHistory: AthleteNote[]` journal desktop uses (`AthleteProfileView`) — now genuinely synced both ways. "+" icon opens an add-note dialog; latest entry shown with timestamp, "Show N more" toggle expands the rest. Fixed a latent bug present on both mobile and desktop: the legacy single-string note was only shown while `notesHistory` was completely empty, so it silently disappeared forever the first time a new-style note was added. `allNotes` now appends the legacy note (tagged "imported") whenever it hasn't been migrated yet, regardless of how many newer entries exist. |
...
| **Athlete app — exercise detail on name click** | In `AthleteSessionPage`, exercise name is a tappable button (overview + active phases, incl. circuit sub-exercises) that opens the exercise detail sheet (video + description). Info icon fully removed everywhere, including circuit sub-exercise rows. **Architecture note:** the athlete's Supabase session has no read access to the coach's exercise library (`custom_libraries` is scoped to the owning coach's `user_id`) — this is *why* the snapshot-at-sync pattern (`exerciseVideoUrl`/`exerciseDescription` on `ExerciseSummary`/`CircuitExerciseSummary`) exists, and an athlete-side live-lookup fallback can never work. `AthleteSessionPage` relies on the snapshot only. `CoachMobileSessionEditPage` — which the coach uses to build/edit sessions and which *can* read the library — now refreshes that snapshot from the live exercise library on every save (for top-level and circuit-member exercises alike), so the athlete-visible snapshot self-heals whenever the coach edits and saves the session. `ExerciseDetailDialog` (`CoachMobileSessionEditPage`) upgraded to show a video thumbnail with play overlay + a "Description" header, matching `CoachMobileSessionLoggingPage`'s sheet. `CircuitExerciseList` (used in both view and edit mode) is now name-tappable too, consistent with everywhere else. `CoachMobileSessionLoggingPage` (in-person logging, used only under the coach's own auth) keeps its working live-lookup fallback — that one is architecturally fine since the coach is the one logged in. |
...
| **Coach mobile — squad overview in athlete list** | Group selector pill chips (All + each group) at top of Athletes tab. Each athlete row shows: avatar, name, today's session name or "Rest day" icon, wellness dot (green/amber/red composite, if wellbeing block active), illness chip (red "Ill", if OSTRC active + athlete reported illness today), pain chip (amber "Pain", if OSTRC active + pain reported + no illness). Check-ins and today's schedule fetched in two batch Supabase queries on load. Connection status dot retained. |
...
| **Coach mobile — tests & events in athlete training calendar** | Amber/blue chips per day card (between intensity and sessions). "+ Test / Event" action button opens `CalendarEventDialog` (reused from desktop). Tapping a chip reopens the dialog for view/delete. Tests with a linked parameter show "Enter result" bottom sheet — writes to `athlete_test_results` (same table as athlete-entered results). Once a result exists for a `parameterId:date` pair, both coach mobile and athlete app show it as locked ("Result: X") and hide the entry button. Coach insert RLS policy added. |
...
| **Day demarcation in weekly calendars** | Horizontal divider line inserted between consecutive day cards in the coach mobile athlete Training tab (`CoachMobileAthleteProfilePage`) and the athlete app Plan tab (`AthletePlanPage`), replacing the plain `space-y-2` gap. Addresses crowding when several sessions + tests/events stack on one day. Desktop `AthleteCalendarView` not touched (column-grid layout, different fix needed if requested). |
| **Wizard auto-save** | `useWizardAutoSave` hook with `markDirty()`, debounced 3 s, wired across MacrocyclePage / MesocyclePage / MicrocyclePlanningPage. `AutoSaveIndicator` in each wizard header. Guard prevents auto-creating empty programs. |
| **Auto-calculated target loads from athlete data** | Formula builder in toolbox gains "Athlete data" badge group (e1RM, body metrics, performance params). Tokens resolved at session-render time per exercise per athlete (Epley e1RM from `athlete_session_logs`, biometrics, performance parameters). `athleteDataRefs` on `ToolboxEntry`; `formulaEvaluator.ts` accepts optional `athleteContext`; resolved values stored in `athlete_schedule` at plan-assignment time. `parseNumeric` handles intensity ranges (e.g. "82–87%" → midpoint 84.5%). Coach can always override calculated values. |
| **Exercise history in session views** | `ExerciseHistorySheet` wired into all four surfaces: `AthleteSessionPage` (athlete active session), `CoachMobileSessionEditPage` + `CoachMobileSessionLoggingPage` (coach mobile), and `WorkoutSessionSheet` (desktop wizard session modal + desktop athlete calendar). History icon on every non-circuit exercise row when an athlete connection is present. Background prefetch on sheet open + DB index on `(athlete_connection_id, date DESC)` for fast loading. Wizard AI assistant also receives the athlete's exercise history as context. |
| **Workout print / PDF** | Printer button in `WorkoutSessionSheet` header calls `window.print()`. `PrintSessionView` component (`src/components/print/PrintSessionView.tsx`) renders semantic HTML: section headers, per-exercise parameter grid (set rows × param columns, `showInGridByDefault` controls visibility), auto-calculated params resolved via 4-pass formula context (toolbox IDs → names → direct param sweep → athlete data refs), non-grid params as pill chips, superset exercises grouped under a "Superset A/B/…" header with 1./2. numbering, "Each side" badge, section + exercise notes. `@media print` visibility trick in `src/styles/print-session.css` hides app chrome and reveals only the print view. Styling by Claude Design: monospace param grid, accent color via `--coach-accent` CSS custom property, logo via `psv-coach-logo` grid placement. Coach branding (logo + accent color) read from `useCoachProfile()` and passed as props — auto-applied on every print. `/dev/print-preview` route with mock data for design iteration. Coach mobile follow-up still open. |
| **Exercise video / description live sync** | When a coach updates `videoUrl` or `description` for an exercise in the library, all existing `athlete_schedule` rows referencing that exercise are automatically patched. `syncExerciseDetailToSchedule` in `src/utils/exerciseDetailSync.ts` fires fire-and-forget after every `updateExerciseInLibrary` call in `CustomLibrariesContext.tsx`. Covers both top-level exercises (`exerciseLibraryId`) and circuit sub-exercises (`exerciseId`). Detects changes via both the top-level `videoUrl`/`description` fields and column-role paths (`role: 'video'` / `role: 'description'`). Batched upserts (200 rows/batch) to `athlete_schedule` with `onConflict: athlete_connection_id,date`. |
| **Anamnesis — athlete intake records** | Coach-configurable templates (Supabase `anamnesis_templates` table, auto-seeded with a 5-section sports-science default, fully editable/deletable). Per-athlete timestamped records (`athlete_anamneses` table) with template snapshot (historical immutability), per-record custom questions, notes, and AI-generated summary (Haiku). Template editor accessible inline from the athlete anamnesis sheet (+ create / pencil edit buttons) — no separate Coach Profile tab needed. `AthleteAnamnesisTab` placed immediately after Profile tab in athlete profile. |
| **Session completion notes forwarded to chat** | Athlete post-session notes (`athlete_session_logs.comment`) are forwarded to the coach↔athlete chat thread as a `session_note` message type. Visible in CompletedSessionSheet on the coach desktop calendar and surfaced in the Messages thread. |
| **Athlete profile notes — inline edit & delete** | Coach can inline-edit or delete individual note entries in the Profile tab. Each note card shows a pencil icon (enters edit mode with Textarea + Save/Cancel) and a trash icon (deletes with `notesHistory` filter). Legacy single-string `athlete.notes` correctly cleared when its migrated entry is deleted. |

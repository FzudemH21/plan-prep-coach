...
| 🟠 Later | **Athlete Profile — Settings tab** — per-athlete coach controls: units, workout visibility range, feature toggles. | ⬜ Open |
| 🟠 Later | **Wire Athlete Profile → Athlete App: Settings** — settings configured by coach flow to athlete app in real time. | ⬜ Open |
...
| 🟡 Soon | **Exercise Progressions & Regressions — coach setup** — define progression/regression chains per exercise in the exercise detail modal. Chain visualised top-to-bottom (progressions above, regressions below current). Drag & drop to reorder, levels auto-renumber. Reverse link automatic: if A is regression of B, B appears as progression of A. | ✅ Done — modal section built, Supabase table with RLS, useExerciseProgressions hook, drag & drop reorder with level sync. Reverse-link: ✅ Done. |
| 🟠 Later | **In-session exercise swap (athlete app)** — "⇅ Adjust" button on each exercise card opens a bottom sheet showing the progression/regression chain. Athlete picks a substitute → optional short reason (stored as exercise-level comment) → exercise replaced for this session only, plan never modified. Swap + reason logged in `athlete_session_logs`. Coach sees swaps in session review. Gated by "Allow athlete to add/replace exercises" setting. | ⬜ Open |
...
| 🟠 Later | **Coach calendar exercise swap** — in the athlete calendar (coach app), right-click or hover menu on a session exercise offers "Replace with progression/regression". Replaces for that date only, original plan template untouched. | ⬜ Open |

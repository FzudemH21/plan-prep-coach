...
| 🟡 Near | **GDPR / health data compliance** — wellness check-ins, pain maps, and illness questionnaires constitute health data under GDPR. Required before any EU-facing launch: (1) Sign Supabase DPA (Data Processing Agreement — available in Supabase dashboard at no cost). (2) Configure Supabase project to use an EU region (Frankfurt). (3) Write a privacy policy covering health data collection, retention period, and deletion rights. (4) Add explicit consent step in `AthleteConnectPage` / monitoring enrollment (coach-side toggle is not sufficient — athlete must actively opt in). (5) Ensure full data deletion cascade when an athlete connection is removed. No app architecture changes required beyond consent UI + confirming cascade deletes work. | ⬜ Open |

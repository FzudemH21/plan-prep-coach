...
| 🔴 Now | **Wizard auto-save** — debounced auto-save (3 s after last edit) of the full wizard state to Supabase. `useWizardAutoSave` hook with `markDirty()` wired into key `useEffect` hooks across MacrocyclePage, MesocyclePage, MicrocyclePlanningPage. `AutoSaveIndicator` component shows "Saving…" / "Saved" / "Save failed" in each wizard header. Guard (`hasSaveableContent`) prevents auto-creating empty programs. | ✅ Done |
...
| 🟡 Near | **Auto-calculated target loads from athlete data** — Extends the existing calculated-parameter system in the toolbox to resolve values from live athlete data, not just from other parameters within the same method. **How it works:** The formula builder in `ParameterManagementDialog` gains a second badge group — "Athlete data" — alongside the existing "Available method parameters" badges. Clicking an athlete-data badge inserts a special token into the formula string (e.g. `Intensity * e1RM`). At session render time the system resolves each token to the athlete's actual value. **Token sources:** (1) `e1RM` — the exercise's own estimated 1RM from `athlete_session_logs`, resolved per exercise per athlete (Squat e1RM ≠ Deadlift e1RM for the same method); (2) body metrics — `Body Weight`, `Max HR`, etc. from the athlete's body metrics tab; (3) performance parameters — `Top Speed`, `FTP`, `VO2max`, etc. dynamically fetched from the coach's parameter database for that athlete. If a value is not yet recorded for an athlete, the field stays blank and the coach fills it manually. **Examples:** `Intensity * e1RM` on "Lower Body Resistance Training – Strength" → Squat row gets 80 % × 120 kg = **96 kg**, Deadlift row gets 80 % × 140 kg = **112 kg**; `Intensity * Max HR` on an endurance method → 75 % × 185 bpm = **139 bpm**. Coach can always override any calculated value. **Code changes:** (1) Add `athleteDataRefs?: string[]` to `ToolboxEntry` to track which athlete-data tokens are used; (2) extend `formulaEvaluator.ts` to accept an optional `athleteContext` map at evaluation time; (3) populate that context at session-sheet render from exercise metrics + body metrics + performance parameters; (4) at plan assignment, compute and store the resolved absolute values per exercise per athlete. This is also the foundation for group/squad programming — one relative prescription, individualised absolute loads per athlete automatically. | ⬜ Open |
...
| 🟡 Near | **Workout print / PDF** — "Print workout" button on individual session cards (athlete calendar + coach mobile). Renders a clean print-optimised HTML view (session name, exercises with all planned parameters, sets/reps/intensity, notes) and calls `window.print()`. Browser handles print-to-PDF. Zero dependencies — pure `@media print` CSS. Scope is single-session output only, not full plan export. | ⬜ Open |
| 🔵 Future | **Pre-publish security hardening** — replace `dangerouslyAllowBrowser`, move API key to Supabase Edge Function, per-coach AI usage metering (request count or token budget per tier), rate limiting, full RLS audit | ⬜ Open |
| 🔵 Future | **Exercise video / description live sync** — currently, video URL + description are snapshotted into `athlete_schedule` at session-edit save time. Goal: when the coach updates an exercise in the library, all existing sessions that reference it should automatically reflect the change without requiring the coach to re-open and re-save each session. Implementation: Supabase database trigger on `custom_libraries` UPDATE that traverses JSONB in `athlete_schedule.sessions` and patches matching `exerciseVideoUrl` / `exerciseDescription` fields. Alternative: client-side batch update in `useToolboxData` / `useExerciseLibrary` after every library save. | ⬜ Open |

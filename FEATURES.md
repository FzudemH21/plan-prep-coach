...
| 🟠 Later | **Exercise Progressions & Regressions** — define progression/regression chains per exercise; athlete can swap exercises in-session; swaps logged and visible to coach. | ⬜ Open |
| 🟠 Later | **In-session exercise swap** — "Need an adjustment?" affordance in athlete app, opens bottom sheet with regression/progression chain. Gated by coach setting. | ⬜ Open |

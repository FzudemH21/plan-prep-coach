...
|---|---|---|
| 🟡 Soon | **Athlete Profile — Analysis tab** — remaining: (1) SD-based individual load baseline in sRPE chart (rolling mean + ±1 SD band, flag weeks >1.5 SD above mean). (2) AI pattern interpreter — receives full time-series data for selected window, narrates load + performance patterns, flags anomalies, suggests next-block adjustments (needs careful system prompt design). Trend lines and lagged correlation on hold — value unclear for now. | 🚧 In Progress |
...
| **Athlete Profile — Analysis tab** | Time-window driven training analysis dashboard. **Internal Load panel:** sRPE bar chart, planned vs. actual RPE toggle, independent date range + granularity controls. **Adherence stat.** **Stimulus & Performance panel:** small-multiples per-method mini line charts, Sets as synthetic parameter, performance outcomes row (merges coach-entered + athlete self-reported values from Supabase), dots only where test recorded (no carry-forward), connectNulls across gaps, independent date range + granularity controls, auto-scaled Y-axis, comma-decimal normalization. |

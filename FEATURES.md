...
| P2 | **Workout completion sync** — after an athlete completes and logs a session in the app (Borg + optional notes saved to `athlete_session_logs`), the session is marked as completed and locked in the athlete app: distinct visual treatment (e.g. green checkmark header, read-only exercise cards — no set editing), no longer launchable as a fresh workout. In the coach's athlete calendar view, completed sessions show a completion indicator with the logged Borg RPE and timestamp. Data source: query `athlete_session_logs` by `athlete_connection_id + date + session_id` and match against the current schedule. Feeds adherence tracking and planned-vs-actual in Plan Review. | ⬜ Open |

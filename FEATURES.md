...
|---|---|---|
| ✅ Done | **Exercise comments & coach feedback** — text comments + media attachments (images, video, documents) on both coach and athlete side. Paperclip button in all three chat UIs (AthleteProfileView embed, CoachMessagesPage ThreadView, AthleteMessagesPage). Files stored in Supabase Storage under `chat/{connectionId}/`. Rendered inline: images as thumbnails, videos as `<video>` player, documents as download card. 7-day signed URLs. Pending-file chips shown before send. | ✅ Done |
| 🟠 Later | **Athlete progress visibility (athlete app)** — athletes can view their own performance and body metric trends in the Profile tab: key performance parameters as line charts over time (same data as coach Performance tab, scoped to their own values), personal bests for main lifts, body metric history. They log the data — they should be able to see it. | ⬜ Open |
| 🟠 Later | **Rest-day message (athlete app Today tab)** — on days with no session, Today tab shows a contextual rest/recovery card instead of a blank screen: confirms it's a planned rest day, shows the next upcoming session (name + date), optional coach note for the day. Keeps the athlete connected to the plan on off-days. | ⬜ Open |

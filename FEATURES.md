...
| 🔵 Future | **Exercise history in session views** — Per-exercise history panel accessible from any session card: wizard session modal, athlete calendar (desktop + coach mobile), and athlete app active session. Shows the last N times this exercise was performed: date, session name, sets × reps × weight (or whatever params were logged), and RPE. Data source: `athlete_session_logs.sets_logged` (already populated). Coach use case: reference previous load when planning. Athlete use case: know what weight to pick before starting a set. Triggered by a small clock/history icon on the exercise row. | ✅ Done — `ExerciseHistorySheet` wired into all four surfaces: `AthleteSessionPage` (athlete active session), `CoachMobileSessionEditPage` + `CoachMobileSessionLoggingPage` (coach mobile), and `WorkoutSessionSheet` (desktop wizard session modal + desktop athlete calendar). History icon appears on every non-circuit exercise row when an athlete connection is present (`athleteConnectionId` prop). |

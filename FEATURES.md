...
| **Athlete progress visibility (athlete app)** | Profile tab gains Overview / Progress sub-tabs. Progress tab: Body Metrics section (from coach-synced snapshot), Performance Parameters section (same), Exercise History section (from `athlete_session_logs` — exercise list → bottom sheet with e1RM area chart + collapsible per-session set tables with per-session column filtering). e1RM tags migrated from localStorage to `exercise_param_tags` Supabase table with RLS (coach manages, athlete can read their coach's tags). Metrics snapshot auto-pushed to `athlete_connections.profile_data` whenever coach adds/removes measurements in the Performance tab. `useAthleteExerciseMetrics` hook for athlete-side data fetching. |

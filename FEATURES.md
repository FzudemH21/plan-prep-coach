...
| **Athlete app — exercise detail on name click** | In `AthleteSessionPage`, exercise name is now a tappable button (overview + active phases, incl. circuit sub-exercises) that opens the existing exercise detail sheet (video + description). Info icon fully removed (was still present on circuit sub-exercise rows). `ExerciseDetailSheet` now falls back to a live lookup in the exercise library by `exerciseLibraryId` when the session's snapshotted `exerciseVideoUrl`/`exerciseDescription` are empty (e.g. exercises added to a manually-created session before/without a snapshot) — fixes "No details available" showing for exercises that do have details configured in the Exercise Database. Same fix + same UI change applied to `CoachMobileSessionLoggingPage` (in-person session logging, overview + active phase + circuits), which previously only had the info-icon button with no live-lookup fallback at all. `CoachMobileSessionEditPage` already had the live-lookup fallback (untouched). |

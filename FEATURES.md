...
| 🟡 Near | **Workout print / PDF** — Print button inside the open `WorkoutSessionSheet` (coach desktop first). Renders a standalone `PrintSessionView` component with semantic HTML: section headers, per-exercise parameter grid (columns = `showInGridByDefault` params, rows = sets), non-grid params as `Param: Value` chips per exercise, session notes, superset labels. `window.print()` + `@media print` CSS hides everything except the print view. Zero runtime dependencies. `/dev/print-preview` route renders `PrintSessionView` with real data for design handoff. All visual styling lives in a single `print-session.css` file — separated from logic for a clean Claude Design pass. Coach mobile extension is a follow-up. | ✅ Done — `PrintSessionView` component + `src/styles/print-session.css`; Printer button in `WorkoutSessionSheet` header calls `window.print()`; `@media print` visibility trick hides app chrome; `/dev/print-preview` dev route with mock data. Coach mobile follow-up still open. |
